...
def findPointSources(fitsFileName):
	...
	pointSourceCatalogName = fitsFileName[:len(
	    fitsFileName) - 5] + "_pointSourceCatalog"
		...
		for wscleanR in robust_arr:
			...
			if wscleanR < 0:
				...
				for wscleanT in taper_arr:
					...
					# Directory for fakesource and observation
					if not os.path.exists('UV'+str(l_name)+'_R'+str(r_name)+'_T'+str(wscleanT)):
						os.makedirs('UV'+str(l_name)+'_R'+str(r_name)+'_T'+str(wscleanT))
					combination_arr.append('UV'+str(l_name)+'_R' +
					                       str(r_name)+'_T'+str(wscleanT))
					# directory only for observation data
					if not os.path.exists('nosources_atrous_UV'+str(l_name)+'_R'+str(r_name)+'_T'+str(wscleanT)):
						os.makedirs('nosources_atrous_UV'+str(l_name) +
						            '_R'+str(r_name)+'_T'+str(wscleanT))
					fakeSourceComb = "fakesource"+str(fakeSource[0])+"mJy_"+str(
					    fakeSource[1])+"arcsec_"+str(fakeSource[2])+"_"+str(fakeSource[3])
					# directory only for fake source data
					if not os.path.exists(fakeSourceComb+'_atrous_UV'+str(l_name)+'_R'+str(r_name)+'_T'+str(wscleanT)):
						os.makedirs(fakeSourceComb+'_atrous_UV'+str(l_name) +
						            '_R'+str(r_name)+'_T'+str(wscleanT))
			else:
				# Directory for fakesource and observation
				if not os.path.exists('UV'+str(l_name)+'_R'+str(r_name)):
					os.makedirs('UV'+str(l_name)+'_R'+str(r_name))
					combination_arr.append('UV'+str(l_name)+'_R'+str(r_name))
				# directory only for observation data
				if not os.path.exists('nosources_atrous_UV'+str(l_name)+'_R'+str(r_name)):
					os.makedirs('nosources_atrous_UV'+str(l_name)+'_R'+str(r_name))
				fakeSourceComb = "fakesource"+str(fakeSource[0])+"mJy_"+str(
				    fakeSource[1])+"arcsec_"+str(fakeSource[2])+"_"+str(fakeSource[3])
				# directory for fakesource data
				if not os.path.exists(fakeSourceComb+'_atrous_UV'+str(l_name)+'_R'+str(r_name)):
		...
		for wscleanR in robust_arr:
			...
			if wscleanR < 0:
				...
				for wscleanT in taper_arr:
					...
					wscleancommand = "wsclean -no-update-model-required -multiscale -beam-size 20 -fits-mask newMask.mask.fits -minuv-l "+str(wscleanL)+".0 -size "+str(wscleanSize)+" "+str(wscleanSize)+" -reorder -weight briggs "+str(
					...
					os.system("mv nosources_atrous_uv"+str(l_name)+"_robust"+str(r_name)+"_taper" +
					          str(wscleanT)+"* nosources_atrous_UV"+str(l_name)+"_R"+str(r_name)+"_T"+str(wscleanT))
					os.system("mv nosources_atrous_UV"+str(l_name)+"_R"+str(r_name)+"_T" +
					          str(wscleanT)+" UV"+str(l_name)+"_R"+str(r_name)+"_T"+str(wscleanT))
			else:
				# no taper-looping...
				...
				# move files to folder and folder to directory
				os.system("mv nosources_atrous_uv"+str(l_name)+"_robust" +
				          str(r_name)+"* nosources_atrous_UV"+str(l_name)+"_R"+str(r_name))
				os.system("mv nosources_atrous_UV"+str(l_name)+"_R" +
				          str(r_name)+" UV"+str(l_name)+"_R"+str(r_name))
		...
		for wscleanR in robust_arr:
			...
			if wscleanR < 0:
				...
				for wscleanT in taper_arr:
					...
					os.system(wscleancommand)
					# move files to folder and folder to directory
					os.system("mv "+str(fakeSourceComb)+"_atrous_uv"+str(l_name)+"_robust"+str(r_name)+"_taper" +
					          str(wscleanT)+"* "+str(fakeSourceComb)+"_atrous_UV"+str(l_name)+"_R"+str(r_name)+"_T"+str(wscleanT))

					os.system("mv "+str(fakeSourceComb)+"_atrous_UV"+str(l_name)+"_R"+str(r_name) +
					          "_T"+str(wscleanT)+" UV"+str(l_name)+"_R"+str(r_name)+"_T"+str(wscleanT))
			else:
				# no taper-looping...
				fakeSourceComb = "fakesource"+str(fakeSource[0])+"mJy_"+str(
				    fakeSource[1])+"arcsec_"+str(fakeSource[2])+"_"+str(fakeSource[3])
				...
				# move files to folder and folder to directory
				os.system("mv "+str(fakeSourceComb)+"_atrous_uv"+str(l_name)+"_robust" +
				          str(r_name)+"* "+str(fakeSourceComb)+"_atrous_UV"+str(l_name)+"_R"+str(r_name))
				os.system("mv "+str(fakeSourceComb)+"_atrous_UV"+str(l_name) +
				          "_R"+str(r_name)+" UV"+str(l_name)+"_R"+str(r_name))
...
def doGaussianFolding(folding_do, inpDataPath, inpDataName, targetbmaj, targetbmin, targetTheta, targetDataName):
	if folding_do:
		targetDataName = gaussianFolding(
		    inpDataPath, inpDataName, targetbmaj, targetbmin, targetTheta, targetDataName)
...
def gaussianFolding(inpDataPath, inpDataName, targetbmaj, targetbmin, targetTheta, targetDataName):
	...
	with fits.open(inpDataPath+'/'+inpDataName) as hdul:
		...
		data = hdul[0].data
		sigmaX = np.sqrt(
		    np.abs(targetSigmaX**2 - (3600*header['BMAJ']/(2.*np.sqrt(2.*np.log(2.))))**2))
		sigmaY = np.sqrt(
		    np.abs(targetSigmaY**2 - (3600*header['BMIN']/(2.*np.sqrt(2.*np.log(2.))))**2))
		...
		data_fold[:][:] *= ((targetbmaj*targetbmin) /
...
def ringIntMain(stepsForRadialProfile, beamx, beamy, FWHMFitMinimum, FWHMFitMaximum, FWHMFitSteps, regionstring, comb, galFWHM, fakeSource, folding_do, targetbmaj, targetbmin, targetTheta):
	...
	writeData(path=outputpath, name="FWHM_plot_fake",
	          xdata=FWHM_fake_fr, ydata=a_fake_fr, yerror=e_fake_fr)
...
def interpretAsArray(inputString):
	out = inputString.replace(']', '').replace('[', '').replace(
	    ' ', '').replace('"', '').split(',')
	return out
